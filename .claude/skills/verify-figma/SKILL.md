...
## ⛔ 절대 규칙 — 이 섹션의 규칙은 예외 없이 반드시 따른다

1. **정확도 99% 이상이 될 때까지 멈추지 않는다.**
2. **정확도는 추정하지 않는다.** 반드시 항목을 세어 정확한 숫자를 계산한다.
3. **99% 미만이면 사용자에게 묻지 않는다.** 자동으로 수정하고 재검증한다.
4. **코드만 보고 "시각적으로 일치한다"고 판정하지 않는다.**
5. **노드 단위가 아니라 요소 단위로 검증한다.**
6. **매 라운드마다 요소별 검증 테이블을 출력한다.**
7. **렌더링 스크린샷은 자동 캡처를 우선한다.** 자동 캡처 실패 또는 사용자 명시 지정(`--screenshot`) 시에만 수동 스크린샷을 허용한다.
8. **검증 완료 후 반드시 임시 파일(스크린샷, diff 이미지)을 정리한다.**
...
연결되지 않은 서버가 있으면 설치/연결 방법을 안내하고 중단한다.
...
#### 3-2. 속성 대조 검증 (자동 스크립트)

생성된 각 코드 파일에 대해 `verify-figma-props.sh`를 실행한다:

```bash
.claude/hooks/verify-figma-props.sh <code-file> /tmp/figma-cache/{fileKey} [nodeId1 nodeId2 ...]
```

이 스크립트가 자동으로 검출하는 항목:
- padding/gap 누락 및 값 불일치 (design-context 대조)
- flex-direction 불일치 (layoutMode 대조)
- border-style 불일치 (strokeDashes + strokes/fills 대조)
- rotation 미적용 (rotation 대조)
- overflow-hidden 누락 (clipsContent 대조)
- layoutSizing 불일치 (FILL/HUG 대조)
- SVG fill 색상 불일치 (fills + SVG 파일 대조)

스크립트 출력의 ❌ 항목은 Phase V5에서 반드시 수정한다.
...
`lint-generated.sh` hook이 코드 작성 시 자동 검출하는 항목 (12개 Rule):
- hex/rgb 하드코딩, inline SVG, 아이콘 라이브러리, placeholder, CSS 아이콘, img alt, line-clamp overflow, 인라인 px, object-fit, bg-border 혼용, SVG 크기 누락
...
같은 요소의 같은 항목이 3라운드 연속 fail → "수정 불가":
- 분모에서 제외
- 전체의 5% 초과 시 사용자에게 알림
